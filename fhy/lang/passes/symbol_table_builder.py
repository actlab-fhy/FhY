...
# TODO: when visitor class automatically visits children, remove the super().visit_xxx(node) call


class SymbolTableBuilder(ast.Visitor):
    """Builds a symbol table for the given AST module and throws an exception if a variable is used before being declared or if a variable is declared more than once in the same namespace
    """
    ...
    @property
    def symbol_tables(self) -> Dict[ir.Identifier, ir.Table[ir.Identifier, ir.SymbolTableFrame]]:
    ...
    def _push_namespace(self, namespace_name_hint: str) -> None:
        ...
        self._symbol_tables[namespace_name] = ir.Table[ir.Identifier, ir.SymbolTableFrame]()
    ...
    def _assert_symbol_not_defined(self, symbol: ir.Identifier) -> None:
        if self._is_symbol_defined(symbol):
            raise AlreadyDeclaredIdentifierException(f"Identifier {symbol.name_hint} is already declared in the current namespace")

    def _assert_symbol_defined(self, symbol: ir.Identifier) -> None:
        if not self._is_symbol_defined(symbol):
            raise UndeclaredIdentifierException(f"Identifier {symbol.name_hint} is not declared in the current namespace")
    ...
    def _add_symbol(self, symbol: ir.Identifier, frame: ir.SymbolTableFrame) -> None:
        if len(self._table_stack) == 0:
            raise ValueError("Expected a current table to be set before adding a symbol to it")
        self._table_stack.peek()[symbol] = frame

    def __call__(self, node: ast.Module, *args: Any, **kwargs: Any) -> Any:
        if not isinstance(node, ast.Module):
            raise ValueError(f"\"build_symbol_table\" pass expected a \"Module\" node, got {type(node)}")
    ...
    def visit_Module(self, node: ast.Module) -> None:
        ...
        if len(self._table_stack) != 0:
            raise RuntimeError("Expected the table stack to be empty after visiting the module")
    ...
    def visit_Procedure(self, node: ast.Procedure) -> None:
        self._assert_symbol_not_defined(node.name)
        proc_frame = ir.FunctionSymbolTableFrame(name=node.name, signature=[(arg.qualified_type.type_qualifier, arg.qualified_type.base_type) for arg in node.args])
    ...
    def visit_Operation(self, node: ast.Operation) -> None:
        self._assert_symbol_not_defined(node.name)
        op_frame = ir.FunctionSymbolTableFrame(name=node.name, signature=[(arg.qualified_type.type_qualifier, arg.qualified_type.base_type) for arg in node.args] + [(node.return_type.type_qualifier, node.return_type.base_type)])
    ...
    def visit_Argument(self, node: ast.Argument) -> None:
        arg_frame = ir.VariableSymbolTableFrame(name=node.name, type=node.qualified_type.base_type, type_qualifier=node.qualified_type.type_qualifier)
        ...
        if isinstance(node.qualified_type.base_type, ir.NumericalType):
            ...
            for dimension in shape_dimension_identifiers:
                if not self._is_symbol_defined(dimension):
                    var_frame = ir.VariableSymbolTableFrame(name=dimension, type=ir.NumericalType(ir.DataType(ir.PrimitiveDataType.INT)), type_qualifier=ir.TypeQualifier.PARAM)
    ...
    def visit_DeclarationStatement(self, node: ast.DeclarationStatement) -> None:
        ...
        var_frame = ir.VariableSymbolTableFrame(name=node.variable_name, type=node.variable_type.base_type, type_qualifier=node.variable_type.type_qualifier)
...
def build_symbol_table(node: ast.ASTNode) -> Dict[ir.Identifier, ir.Table[ir.Identifier, ir.SymbolTableFrame]]:
