...
@pytest.mark.xfail(reason="Retrieving an ASTNode", raises=directory.UnregisteredASTNode)
...
def test_register_ast_node(setup_ast_node):
    ...
    assert issubclass(setup_ast_node, ASTNode), "Expected GenericNode to be a Sublcass of ASTNode"
    assert setup_ast_node in directory._ast_node_types, "Expected Generic Node to be Registered"

    ret = directory.get_ast_node_type_info(setup_ast_node)
    assert ret.fields == {"_span": Span, "x": int, "y": float}, "Expected Correct Annotations of GenericNode"


def test_register_ast_node_with_bad_subclassing(setup_bad_ast_nodes):
    assert issubclass(setup_bad_ast_nodes, ASTNode), "Expected GenericNode to be a Sublcass of ASTNode"
    assert setup_bad_ast_nodes in directory._ast_node_types, "Expected Generic Node to be Registered"
