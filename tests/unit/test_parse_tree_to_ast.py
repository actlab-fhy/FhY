...
class ExpressionExactEqualityGetter(BasePass):
    ...
    def visit_PrimitiveDataType(self, node: ir.PrimitiveDataType, other: Any) -> bool:
        if not isinstance(other, ir.PrimitiveDataType):
            return False
        return node.core_data_type == other.core_data_type

    def visit_TemplateDataType(self, node: ir.TemplateDataType, other: Any) -> bool:
        if not isinstance(other, ir.TemplateDataType):
            return False
...
def _create_identifier_map(node: ast_node.ASTNode) -> dict[str, ir.Identifier]:
...
def _assert_is_expected_operation(
    ...
    assert_type(node.name, ir.Identifier, "operation name")
...
def _assert_is_expected_argument(
    ...
    assert_type(node.name, ir.Identifier, "argument name")
...
def _assert_is_expected_numerical_type(
    ...
    expected_core_data_type: ir.CoreDataType,
    expected_shape: list[ast_node.Expression],
) -> None:
    assert_type(numerical_type, ir.NumericalType, "numerical type")
    assert_type(numerical_type.data_type, ir.DataType, "numerical type data type")
    assert numerical_type.data_type.core_data_type == expected_core_data_type
    assert_sequence_type(
        numerical_type.shape, ast_node.Expression, "numerical type shape"
    )
    assert len(numerical_type.shape) == len(expected_shape)
    for i, shape_component in enumerate(numerical_type.shape):
        assert _is_expressions_exactly_equal(shape_component, expected_shape[i]), (
...
def _assert_is_expected_shape(
    ...
    assert_sequence_type(shape, ast_node.Expression, "shape")
...
def _assert_is_expected_index_type(
    ...
    expected_low: ast_node.Expression,
    expected_high: ast_node.Expression,
    expected_stride: ast_node.Expression | None,
) -> None:
    assert_type(index_type, ir.IndexType, "index type")
    assert_type(index_type.lower_bound, ast_node.Expression, "index type lower bound")
    assert _is_expressions_exactly_equal(index_type.lower_bound, expected_low), (
        "Expected lower bound to be equal "
        + f"(expected: {expected_low}, actual: {index_type.lower_bound})"
    )
    assert_type(index_type.upper_bound, ast_node.Expression, "index type upper bound")
    assert _is_expressions_exactly_equal(index_type.upper_bound, expected_high), (
        "Expected upper bound to be equal "
        + f"(expected: {expected_high}, actual: {index_type.upper_bound})"
    )
    if expected_stride is not None:
        assert_type(index_type.stride, ast_node.Expression, "index type stride")
        assert _is_expressions_exactly_equal(index_type.stride, expected_stride), (
...
def _assert_is_expected_declaration_statement(
    ...
    assert_type(node.variable_name, ir.Identifier, "variable name")
...
@pytest.mark.parametrize(
    ...
    [
        ("input", ir.TypeQualifier.INPUT),
        ("output", ir.TypeQualifier.OUTPUT),
        ("temp", ir.TypeQualifier.TEMP),
        ("param", ir.TypeQualifier.PARAM),
        ("state", ir.TypeQualifier.STATE),
    ],
)
def test_empty_procedure_with_scalar_argument_qualifiers(
    construct_ast, type_qualifier: str, expected_type_qualifier: ir.TypeQualifier
):
    ...
    arg_qualified_type = arg.qualified_type
    _assert_is_expected_qualified_type(
        arg_qualified_type, expected_type_qualifier, ir.NumericalType
    )
    arg_base_type = arg_qualified_type.base_type
    _assert_is_expected_numerical_type(arg_base_type, ir.CoreDataType.INT32, [])
...
@pytest.mark.parametrize(
    ...
    [
        ("uint8", ir.CoreDataType.UINT8),
        ("uint16", ir.CoreDataType.UINT16),
        ("uint32", ir.CoreDataType.UINT32),
        ("uint64", ir.CoreDataType.UINT64),
        ("int8", ir.CoreDataType.INT8),
        ("int16", ir.CoreDataType.INT16),
        ("int32", ir.CoreDataType.INT32),
        ("int64", ir.CoreDataType.INT64),
        ("float16", ir.CoreDataType.FLOAT16),
        ("float32", ir.CoreDataType.FLOAT32),
        ("float64", ir.CoreDataType.FLOAT64),
        ("complex64", ir.CoreDataType.COMPLEX64),
        ("complex128", ir.CoreDataType.COMPLEX128),
    ],
)
def test_empty_procedure_with_scalar_argument_data_types(
    construct_ast, data_type: str, expected_core_data_type: ir.CoreDataType
):
    """Test an empty procedure with a single scalar argument with varying
    data types.
    """
...
-> output int32 {{}}"
    ...
    for j, k in zip(operation.templates, templates):
        assert_type(j, ir.TemplateDataType, "template type")
...
def test_operation_template_type_body(construct_ast):
    ...
    assert_type(template, ir.TemplateDataType, "template type")
    arg_base_type = operation.args[0].qualified_type.base_type
    assert_type(arg_base_type, ir.NumericalType, "numerical type")
    assert_type(arg_base_type.data_type, ir.TemplateDataType, "template type")
    assert_name(
...
"""
    ...
    _assert_is_expected_declaration_statement(
        ...
        ast_node.FunctionExpression(
            ...
            template_types=[ir.TemplateDataType(data_type=identifier_map["P"])],
    ...
    _assert_is_expected_declaration_statement(
        ...
        ast_node.FunctionExpression(
            ...
            template_types=[ir.PrimitiveDataType(core_data_type=ir.CoreDataType.INT32)],
            args=[],
        ),
    )
...
def test_array_declaration_statement(construct_ast):
    ...
    _assert_is_expected_qualified_type(
        array_type, ir.TypeQualifier.TEMP, ir.NumericalType
    )
    _assert_is_expected_shape(
...
def test_index_variable_declaration_statement(construct_ast):
    ...
    _assert_is_expected_qualified_type(index_type, ir.TypeQualifier.TEMP, ir.IndexType)
