...
.. code-block:: text
   ...
   proc matmul(input int32[m, n], input int32[n, p] B, output int32[m, p] C){
      temp index[1:m] i;
      temp index[1:p] j;
      temp index[1:n] k;
...
In the current ``proc`` routines, all assignments are *non-blocking*.
This means that the assignments are executed in parallel and the order of execution is not guaranteed.
However, in ``op`` routines, all assignments are *blocking*.
This means that the assignments are executed sequentially and the order of execution is guaranteed.
Moving on, the ``state`` type qualifier acts in the same way as a ``static`` variable in a C/C++ function.
The value of a ``state`` variable is preserved across invocations of the procedure.
Finally, the ``native`` routine is used to define operations that are not implemented in *FhY* and are implemented in another language (e.g., C/C++).
...
.. code-block:: text
   ...
   op backward_propagation(input float32[n] x, input float32[m, n] W, input float32[m] b, input float32[m] y) -> output (float32[m, n], float32[m]) {
      ...
      dW[i, j] = (2 / m) * x[j] * y_hat[i] * (y_hat[i] - y[i]) * (1 - y_hat[i]);
      db[i] = (2 / m) * y_hat[i] * (y_hat[i] - y[i]) * (1 - y_hat[i]);
      return (dW, db);
   }
