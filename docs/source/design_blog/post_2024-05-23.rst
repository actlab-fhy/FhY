...
`pip install -e <package>`). And because this isn't how clients will use your package, I
separated unit testing only (in normal installed environment), and a separate coverage
environment using the pytest-cov plugin in development mode. But this setup is less
than ideal, because it means we are running pytest twice.
...
It's easy to forget when using tox, that the local copy of your source code is visible
...
To wrap up our coverage woes, several other configurations need to change to package
correctly using setuptools, and the coverage tool. In our pyproject.toml:
...
Now at this point, your whole configuration should accomplish and correctly capture
coding coverage of your project, using tox virtual environments, even if you need to
also capture subprocess, or multiple parallel processes.
...
#. Use `coverage.py` directly when monitoring code coverage, forgoing the pytest plugin,
   to separate unit testing and coverage reporting.
...
#. Perform "style" checks, confirming code linting, formatting, static typing, and
   ...
   environment, we can matrix python versions and Host OS platforms, to perform pytest
